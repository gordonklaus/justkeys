module Voronoi.Delaunay.BowyerWatson exposing (addPoint)

import Voronoi.Delaunay.Triangle exposing (containsPoint)
import Voronoi.Geometry.Edge
import Voronoi.Geometry.Triangle
import Voronoi.Model exposing (DelaunayTriangle, Edge, Model, Point, Triangle)
...
addPoint point triangles =
    if triangles == [] then
        goodTriangles point Voronoi.Delaunay.Triangle.defaultTriangles
            |> retriangulatePolygonalHole point (badTriangleEdges point Voronoi.Delaunay.Triangle.defaultTriangles)
    else
...
retriangulatePolygonalHole point edges triangles =
    List.append
        ...
        (List.map
            (\edge ->
                Voronoi.Geometry.Triangle.getDelaunayTriangle (Voronoi.Geometry.Triangle.retriangulate point edge)
            )
            edges
        )
...
badTriangleEdges point triangles =
    List.map
        (\tri -> Voronoi.Geometry.Triangle.getEdges tri.triangle)
        (badTriangles point triangles)
        |> List.concat
        |> Voronoi.Geometry.Edge.getUnique
