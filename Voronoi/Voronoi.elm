module Voronoi.Voronoi exposing (..)

import Svg exposing (Svg, polyline)
import Svg.Attributes exposing (..)
import Voronoi.Delaunay.Triangle
import Voronoi.Geometry.Edge
import Voronoi.Model exposing (DelaunayTriangle, Edge, Model, Point, VoronoiPolygon)
...
draw model =
    let
        voronoi =
            get model
    in
    --List.map Voronoi.Geometry.Edge.draw edges
...
toString voronoi =
    String.concat (List.intersperse " " (List.map (\edge -> Voronoi.Geometry.Edge.toString edge) voronoi.edges))
...
getVoronoiRecurse voronoi triangle triangles =
    let
        neighbors =
            Voronoi.Delaunay.Triangle.neighbors triangle triangles
    in
    List.append
        [ VoronoiPolygon (List.filterMap (connectTriangles triangle) neighbors) Nothing ]
...
connectTriangles a b =
    if Voronoi.Delaunay.Triangle.isNeighbor a b then
        Maybe.map2
            (\centerA centerB -> Edge (Point centerA) (Point centerB))
            a.circle.center
            b.circle.center
    else
        Nothing
