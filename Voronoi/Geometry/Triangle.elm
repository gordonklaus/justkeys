module Voronoi.Geometry.Triangle exposing (..)

import Color
import Math.Vector2 exposing (Vec2, getX, getY, vec2)
import Svg exposing (Svg, polyline)
import Svg.Attributes exposing (..)
import Voronoi.ColorHelper
import Voronoi.Geometry.Distance
import Voronoi.Geometry.Edge
import Voronoi.Geometry.Point
import Voronoi.Geometry.Util
import Voronoi.Model exposing (Edge, Point, Triangle)
...
circumcenter triangle =
    let
        ...
        slopeAB : Maybe Float
        slopeAB =
            Voronoi.Geometry.Util.perpendicularSlope a b

        slopeInterceptAB : Maybe Float
        slopeInterceptAB =
            Voronoi.Geometry.Util.solveSlopeInterceptForB (Voronoi.Geometry.Util.midpoint a b) slopeAB

        -- BC
        slopeBC : Maybe Float
        slopeBC =
            Voronoi.Geometry.Util.perpendicularSlope b c

        slopeInterceptBC : Maybe Float
        slopeInterceptBC =
            Voronoi.Geometry.Util.solveSlopeInterceptForB (Voronoi.Geometry.Util.midpoint b c) slopeBC
...
toString tri =
    List.map Voronoi.Geometry.Point.toString [ tri.a, tri.b, tri.c, tri.a ]
        |> List.intersperse " "
        |> String.concat
...
hasEdge triangle edge =
    List.any (Voronoi.Geometry.Edge.isEqual edge) (getEdges triangle)
...
-}
getDelaunayTriangle : Triangle -> Voronoi.Model.DelaunayTriangle
getDelaunayTriangle tri =
    let
        circCenter =
            findCircumcenter tri
    in
    Voronoi.Model.Circle
        circCenter
        (Voronoi.Geometry.Distance.distanceEuclidean (Maybe.withDefault (vec2 0 0) circCenter) tri.a.pos)
        |> Voronoi.Model.DelaunayTriangle tri
